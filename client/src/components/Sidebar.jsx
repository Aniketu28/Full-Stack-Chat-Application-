...
const Sidebar = ({SelectedUser,setSelectedUser}) => {
  ...
  return (
    <div className={`bg-[#8185B2]/10 min-h-screen p-5 rounded-r-xl overflow-y-scroll text-white ${SelectedUser ? 'max-md:hidden' : ''}`}>
        <div className='pb-5'>
           <div className='flex justify-between items-center'>
              <img src={assets.logo} alt="logo" className='max-w-40' />
              <div className='relative py-2 group'>
                 <img src={assets.menu_icon} alt="menu_icon" className='max-h-5 cursor-pointer'/>
                 <div className='absolute top-full right-3 z-20 w-32 p-5 rounded-md bg-[#282142 border border-gray-600 text-gray-100 hidden group-hover:block'>
           ...
           <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
              ...
              <input type="text" className='bg-transparent border-none outline-none
               text-white text-xs placeholder:text-[#c8c8c8] flex-1' placeholder='Search User..'/>
           </div>
        </div>
        <div className='flex flex-col'>
            {userDummyData.map((user,index)=>(
               <div onClick={()=> {setSelectedUser(user)}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${SelectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
                  <img className='w-[35px] aspect-square rounded-full'
                   src={user.profilePic || assets.avatar_icon} alt="user_profile" />
